...
public class ClienteService : IClienteService
{
        ...
        CadastrarClienteRequest request)
    {
        if (request.Senha != request.ConfirmacaoSenha)
            throw new ClienteValidacaoException(
        ...
        if (cpfDuplicado || emailDuplicado)
            throw CriarExcecaoDeConflito(
                cpfDuplicado,
                emailDuplicado);

        var cliente = new Cliente
        {
            // Placeholder único e temporário.
            // O código definitivo depende do Id gerado pelo banco.
            ...
            EnderecoCobranca =
                CriarEndereco(request.EnderecoCobranca),
        };

        // Endereço de entrega inicial.
        cliente.EnderecosEntrega.Add(
            CriarEnderecoEntrega(request.EnderecoEntrega));
        ...
        try
        {
            ...
            // Código definitivo:
            // CLI-<ano>-<id com 6 dígitos>
            cliente.CodigoCliente =
                $"CLI-{DateTime.UtcNow:yyyy}-{cliente.Id:D6}";
    ...
    public async Task<IReadOnlyList<ClienteResponse>> ConsultarAsync(
        string? codigo,
        string? nome,
        string? cpf,
        string? email)
    {
        ...
        if (!string.IsNullOrWhiteSpace(codigo))
        {
            ...
            query = query.Where(
                c => c.CodigoCliente == codigoNormalizado);
        }
        ...
        if (!string.IsNullOrWhiteSpace(nome))
        {
            ...
            query = query.Where(
                c => c.Nome.ToLower().Contains(nomeNormalizado));
        }
        ...
        if (!string.IsNullOrWhiteSpace(email))
        {
            ...
            query = query.Where(
                c => c.Email.ToLower().Contains(emailNormalizado));
        }
    ...
    {
        var cliente =
            await BuscarClienteCompletoAsync(id);
        ...
        AtualizarClienteRequest request)
    {
        ...
        // O próprio cliente não conta como duplicidade.
        var cpfDuplicado =
        ...
        cliente.TelefoneTipo =
            request.TelefoneTipo.Trim();

        cliente.DDD =
            SomenteDigitos(request.DDD);

        cliente.TelefoneNumero =
            SomenteDigitos(request.TelefoneNumero);

        cliente.Email =
            email;

        // Apenas o endereço de cobrança é alterado aqui.
        // Endereços de entrega possuem endpoints próprios.
        PreencherEndereco(
            cliente.EnderecoCobranca,
            request.EnderecoCobranca);
    ...
    public async Task<ClienteResponse> AdicionarEnderecoEntregaAsync(
        int clienteId,
        EnderecoEntregaRequest request)
    {
        var cliente =
            await BuscarClienteCompletoAsync(clienteId);
    ...
    public async Task<ClienteResponse> AtualizarEnderecoEntregaAsync(
        int clienteId,
        int enderecoId,
        EnderecoEntregaRequest request)
    {
        ...
        if (endereco is null)
            throw new ClienteNaoEncontradoException(
                $"Endereço de entrega com Id {enderecoId} " +
                "não encontrado para este cliente.");
    ...
    string senha)
{
    ...
    if (!cliente.Ativo)
        throw new ClienteConflitoException(
    ...
    // Determina, pela constraint violada,
    // se o conflito foi de CPF, e-mail ou ambos.
    private static ClienteConflitoException
        CriarExcecaoDeConflito(PostgresException ex)
    {
        var constraint =
        ...
        // Se não for possível identificar o campo,
        // assume conflito em ambos por segurança.
        if (!cpfDuplicado && !emailDuplicado)
        {
            cpfDuplicado = true;
            emailDuplicado = true;
        ...
        return CriarExcecaoDeConflito(
            cpfDuplicado,
            emailDuplicado);
    }

    private static ClienteConflitoException
        CriarExcecaoDeConflito(
            bool cpfDuplicado,
            bool emailDuplicado)
    {
        ...
        return new ClienteConflitoException(
            mensagem,
            campos);
    }

    private static string SomenteDigitos(
        string valor) =>
    ...
    private static Endereco CriarEnderecoEntrega(
        EnderecoEntregaRequest request)
    {
        var endereco =
    ...
    private static ClienteResponse MapearParaResponse(
        Cliente cliente) =>
        new(
            cliente.Id,
            cliente.CodigoCliente,
            ...
            cliente.EnderecosEntrega
                .Select(MapearEndereco)
                .ToList());

    private static EnderecoResponse MapearEndereco(
        Endereco endereco) =>
        new(
            endereco.Id,
            endereco.Nome,
